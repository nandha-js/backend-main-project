...
export const createProperty = async (req, res) => {
  const {
    ...
    type, size, rooms, bedrooms, bathrooms, images
  } = req.body;

  if (!title || !price || !address) {
    return res.status(400).json({ success: false, message: 'Please provide title, price, and address' });
  }
  ...
  try {
    ...
    if (!loc || !loc.length || !loc[0].formattedAddress) {
